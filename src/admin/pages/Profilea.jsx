...
function Profilea() {
    ...
    // fetch data from api 
    const fetch = async () => {
        const res = await axios.get(`http://localhost:3000/auser/${localStorage.getItem('userid')}`);
    ...
    const editdata = async (id) => {
        const res = await axios.get(`http://localhost:3000/auser/${id}`);
        console.log(res.data);
      ...
      const submithandel = async (e) => {
        ...
        if(validation())
        {
          const res = await axios.patch(`http://localhost:3000/auser/${formvalue.id}`,formvalue);
          console.log(res);
          if(res.status==200)
          {
